...
- `tech/runbook_whatsapp_bridge_ciclo_vida.md` — el bridge es un LaunchAgent (`RunAtLoad`+`KeepAlive`): lo arranca launchd en cada login, no la app. Dónde vive el estado de encendido (override store de launchd, no la config), escalera enable/bootstrap/kickstart y mensajes de launchd medidos. Leer antes de tocar el arranque/parada del bridge o al diagnosticar "se ha activado solo" (17-08-2026).
...
- `bugs/bug_espejo_teclado_movil_duplicado_2026_08_21.md` — el espejo duplicaba lo escrito desde Android: xterm entrega tecla a tecla y un IME COMPONE (ningún atributo lo arregla). El móvil pasa a escribir en una barra y mandar el texto entero. Leerla antes de tocar la entrada del espejo o de montar input móvil hacia un PTY.
...
- `bugs/bug_pref_proyecto_cwd_home_2026_08_15.md` — una pref "por proyecto" a punto de guardarse contra `/Users/isabel` (`resolveProjectCwd()` cae a `ptyCwd()`). Leer antes de persistir CUALQUIER preferencia atada a una carpeta, y antes de maquetar una casilla: trae el `<label for>` con el input dentro (doble toggle, "clico y no pasa nada") y la trampa del CDP hablando con una instancia vieja (15-08-2026).
