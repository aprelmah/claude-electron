...
- **Limitación v1 del modo voz: no toma `session.relayActive`, solo lo respeta.** Un encargo por voz se rechaza (`la sesión está ocupada con otro turno`) si hay un turno de Telegram en vuelo, pero NO marca el flag mientras dura el suyo, así que un turno de Telegram que llegue a mitad de un turno de voz sí puede interleavarse en el mismo PTY. Tomarlo obligaría a soltarlo desde el vigía del turno, que vive en otro módulo, y un flag mal soltado deja el relay de Telegram esperando 30 s por nada. Si algún día el modo voz toma el flag, el `finally` tiene que estar en el mismo sitio que lo toma.
