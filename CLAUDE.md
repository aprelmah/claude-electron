...
## 🚦 MODO VOZ — código y documentación cerrados, falta la prueba humana (2026-08-05)

Rama `feat/modo-voz`. Las 8 tareas de implementación y los pasos 1, 2 y 4 de la Tarea 9 (documentación) están cerrados: **943 tests (937 pass / 0 fail / 6 skip)**, árbol limpio, sin push.

- Plan: `docs/superpowers/plans/2026-08-04-voz-en-directo.md` (9 tareas TDD, 50 pasos). Spec: `docs/superpowers/specs/2026-08-04-voz-en-directo-design.md` (el porqué, con las latencias **medidas** en este Mac).
- Reglas técnicas, desviaciones reales del plan y pendientes conocidos: sección **"## Modo voz"** más abajo en este mismo fichero, y `.claude/memory/tech/tech_modo_voz.md`.
- **Falta el paso 3 de la Tarea 9: la prueba manual con micrófono.** Eso no lo cierra un agente — el permiso lo concede un humano en un diálogo de macOS, y el barge-in hay que oírlo. Checklist completo, en español llano, para Luismi: **`CHECKLIST-VOZ-MANUAL.md`** (raíz del repo).
- Antes de pasarle el checklist a Luismi falta: review final de la rama y `npm run deploy`.
...
- **Regla para spawns nuevos**: cualquier spawn de PTY nuevo debe decidir explícitamente si pasa por `ensureSessionWorkspace`/`prepareSessionWorkspace` (aislado) o queda excluido — y documentarlo aquí. No dejarlo implícito. `respawnAfterCliUpdate` (reinicio tras auto-update del CLI) **no** vuelve a llamar a `ensureSessionWorkspace`: reutiliza el `session.gitWorkspace` ya creado, así que sigue en el mismo worktree y la misma rama. El **modo voz** (`main/voice-send-target.js`) **no spawnea ningún PTY propio**: el modo "encargo" escribe en el PTY de la sesión madre (aislado como ella) y el modo "charla" reutiliza `subchat-pty` (excluido a propósito, hereda el `workCwd` de la madre). El helper Swift de voz no es un PTY y no toca el repo.
- **PENDIENTE INMEDIATO (no cerrado) — exclusión mutua entre el modo voz y Telegram sobre el mismo PTY.** Hoy el encargo por voz solo **lee** `session.relayActive`: se rechaza (`la sesión está ocupada con otro turno`) si hay un turno de Telegram en vuelo, pero no marca nada mientras dura el suyo. **El orden que queda desprotegido es el más probable**: un turno de voz dura decenas de segundos, así que "llega un Telegram a mitad de un turno de voz" pasa mucho más a menudo que el contrario, y es justo el que no se cubre — los dos textos se entremezclan en el mismo PTY. No corrompe nada (el TUI encola), pero da respuestas cruzadas. Lo que hay que implementar es un **cerrojo con caducidad**, no tomar `relayActive`: `session.voiceTurnUntil = Date.now() + 180000`, que `relayThroughPty` respeta igual que `relayActive` y que **caduca solo** — da exclusión mutua sin el riesgo de flag pegado que tendría un booleano que hay que soltar desde el vigía del turno (otro módulo, cuatro caminos de salida).
...
- **El eco lo cancela `setVoiceProcessingEnabled(true)`** (VoiceProcessingIO de CoreAudio, dentro del `.swift`). Verificado con el TTS sonando por altavoz y el micro abierto: el reconocedor no se oyó a sí mismo. Manos libres sin auriculares es viable.
...
- **La app no está firmada**: el permiso de micrófono/reconocimiento se ancla a la firma ad-hoc, así que **es probable que macOS lo vuelva a pedir en cada `npm run deploy`**. Mismo patrón que las notificaciones nativas en Electron 43.
- **Seis módulos en `main/`, cada uno con un trabajo**: `voice-helper-process.js` (proceso hijo + protocolo), `voice-session.js` (máquina de estados `idle→listening→thinking→speaking`, dueña del helper), `voice-router.js` (charla vs encargo por patrones, no clasificador — un toggle manual en la UI siempre gana a la detección), `voice-send-target.js` (dónde escribe cada turno y qué transcript vigilar — aquí viven las trampas del fork del sessionId, ver abajo), `voice-speakable.js` (markdown → prosa hablable: fuera código, diffs, tablas, URLs — si no queda nada, no se habla), `voice-turn-watcher.js` (vigila el `.jsonl` hasta `turnComplete` reutilizando `main/relay-transcript-helpers.js`). UI: `voice-ui-state.js` en la raíz (lógica pura evento→acción, sin DOM ni IPC — `renderer.js` no se testea en ningún sitio del repo, así que la lógica que importa se extrae siempre a un módulo aparte).
...
- **El micro NO se cierra mientras habla — solo mientras piensa.** Desviación consciente del diseño original (que pedía cerrado en los dos estados): el barge-in (`user-interrupt`) sale del callback del tap de audio del micrófono (`onAudio` en el Swift), y con el micro cerrado ese callback no se ejecuta nunca — el barge-in sería código muerto. Mientras PIENSA (la ventana larga, de segundos a minutos) el micro sí se cierra, que es donde importaba el riesgo de ruido de sala. **Riesgo residual asumido**: con ruido de sala constante por encima del umbral RMS (0,012) mientras habla, la app se auto-interrumpe sola. Es audible al instante y se recupera solo — punto obligatorio del checklist manual.
...
- **`cli.voiceId` se persiste** (Configuración, whitelist `SAFE_CLI`) y se manda al helper tras cada `enable()` — **pero no hay selector en la UI** todavía; hoy solo se puede fijar editando el config JSON a mano. Las voces es-ES instaladas de fábrica son todas `default` (suenan a robot); mejora descargando *Mónica (Mejorada)* a mano desde Ajustes del sistema → Accesibilidad → Contenido hablado → Gestionar voces.
...
- **PENDIENTE, sin verificar**: `extraResources` (el binario `resources/voice-helper` empaquetado) no se ha comprobado con un `npm run dist` real, solo vía los hooks `pre*` de `build:zip`/`dist`. Si el bit de ejecución no sobrevive a la copia, el helper falla al arrancar y el botón sale `broken` solo en la app empaquetada, nunca en dev. Y solo se ha compilado `x86_64` (este Mac es Intel); un build `arm64` empaquetaría el binario equivocado sin avisar.
