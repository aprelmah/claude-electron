...
contextBridge.exposeInMainWorld('tasksAPI', {
  ...
  resetSession:(id)        => ipcRenderer.invoke('tasks:reset-session', { id }),
  sessionLinks:(sessionId) => ipcRenderer.invoke('tasks:session-links', { sessionId }),
  openSession: (payload)   => ipcRenderer.invoke('app:open-task-session', payload),
  ...
  getSkill: (id, cwd) => ipcRenderer.invoke('skills:get', { id, cwd }),
  saveSkill: (payload) => ipcRenderer.invoke('skills:save', payload),
  delegationsList: () => ipcRenderer.invoke('delegations:list'),
  delegationsStart: (payload) => ipcRenderer.invoke('delegations:start', payload || {}),
  delegationsCancel: (id) => ipcRenderer.invoke('delegations:cancel', { id }),
  onDelegationUpdated: listener('delegation:updated'),
  onDelegationProgress: listener('delegation:progress'),
