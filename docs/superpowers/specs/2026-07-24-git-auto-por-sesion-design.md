...
- **Archivos gitignored creados en la sesión se pierden al finalizar.** `git worktree remove --force` borra el worktree entero, incluidos artefactos ignorados por `.gitignore` (builds, `.env` locales, `node_modules` generados en la sesión). El commit de rescate (`add -A`) respeta el gitignore, así que esos archivos no viajan a la rama. Limitación asumida: lo ignorado se considera regenerable.
- **Sesiones Codex nacidas en worktree no aparecen en el historial del proyecto.** El índice de sesiones Codex bucketiza por el cwd del rollout: una sesión que corrió dentro del worktree queda indexada bajo el path del worktree (ya borrado tras finalize), no bajo el proyecto real. Limitación v1 consciente — no hay equivalente Codex de `copySessionsHome`.
- **El operador LAN no ve el aviso de conflicto si el socket ya cerró.** `finalizeSessionGitWorkspace` intenta enviar el frame `status`/`git-warning` por el WebSocket, pero en el teardown normal el socket suele estar ya cerrado. El trabajo no se pierde (la rama `poweragent/session-*` sobrevive en el Mac y el `console.warn` queda en el log del dueño), pero el operador remoto puede no enterarse del conflicto.
