...
const btnKbFicha = document.getElementById('btn-kb-ficha')
const btnKbShortcut = document.getElementById('btn-kb-shortcut')
const kbModal = document.getElementById('kb-modal')
const kbDivider = document.getElementById('kb-divider')
const kbBody = document.getElementById('kb-body')
const kbShortcutBody = document.getElementById('kb-shortcut-body')
const kbSub = document.getElementById('kb-sub')
const kbToolHeading = document.getElementById('kb-tool-heading')
const kbProgressEl = document.getElementById('kb-progress')
const btnCloseKb = document.getElementById('btn-close-kb')
const btnKbDock = document.getElementById('btn-kb-dock')
const kbToolHeader = document.querySelector('.kb-tool-header')
const kbResizeHandle = document.getElementById('kb-resize-handle')
const kbChatView = document.getElementById('kb-chat-view')
const kbFichaView = document.getElementById('kb-ficha-view')
const kbShortcutView = document.getElementById('kb-shortcut-view')
const kbChatProject = document.getElementById('kb-chat-project')
const kbChatSources = document.getElementById('kb-chat-sources')
const kbChatAllSources = document.getElementById('kb-chat-all-sources')
const kbChatClear = document.getElementById('kb-chat-clear')
const kbChatMessages = document.getElementById('kb-chat-messages')
const kbChatStatus = document.getElementById('kb-chat-status')
const kbChatForm = document.getElementById('kb-chat-form')
...
let kbChatHistory = []
let kbChatSourceSelection = new Set()
let kbChatSourceSelectionProject = ''
let kbActiveTool = 'chat'
let kbFloating = false
let kbWindowState = null

const KB_WINDOW_STATE_KEY = 'poweragent.knowledge-window.v1'

function kbReadWindowState() {
  try {
    const saved = JSON.parse(localStorage.getItem(KB_WINDOW_STATE_KEY) || 'null')
    if (saved && typeof saved === 'object') return saved
  } catch {}
  return { left: 260, top: 100, width: 720, height: Math.min(720, Math.max(420, window.innerHeight - 150)), floating: false }
}

function kbClampWindowState(state) {
  const minWidth = 390
  const minHeight = 360
  const maxWidth = Math.max(minWidth, window.innerWidth - 24)
  const maxHeight = Math.max(minHeight, window.innerHeight - 46)
  state.width = Math.min(Math.max(Number(state.width) || 720, minWidth), maxWidth)
  state.height = Math.min(Math.max(Number(state.height) || 620, minHeight), maxHeight)
  state.left = Math.min(Math.max(Number(state.left) || 12, 8), Math.max(8, window.innerWidth - state.width - 8))
  state.top = Math.min(Math.max(Number(state.top) || 42, 34), Math.max(34, window.innerHeight - state.height - 8))
  if (state.dockWidth) state.dockWidth = Math.min(Math.max(Number(state.dockWidth) || 0, 420), Math.round(window.innerWidth * 0.68))
  return state
}

function kbSaveWindowState() {
  if (!kbWindowState) return
  try { localStorage.setItem(KB_WINDOW_STATE_KEY, JSON.stringify({ ...kbWindowState, floating: kbFloating })) } catch {}
}

function kbApplyWindowState() {
  if (!kbModal) return
  kbWindowState = kbClampWindowState(kbWindowState || kbReadWindowState())
  kbModal.classList.toggle('kb-floating', kbFloating)
  if (kbFloating) {
    kbModal.style.left = `${kbWindowState.left}px`
    kbModal.style.top = `${kbWindowState.top}px`
    kbModal.style.width = `${kbWindowState.width}px`
    kbModal.style.height = `${kbWindowState.height}px`
    kbModal.style.right = 'auto'
    kbModal.style.bottom = 'auto'
    kbModal.style.flexBasis = ''
  } else {
    kbModal.style.left = ''
    kbModal.style.top = ''
    kbModal.style.width = ''
    ...
    kbModal.style.flexBasis = kbWindowState.dockWidth ? `${kbWindowState.dockWidth}px` : ''
  }
  if (kbDivider) kbDivider.classList.toggle('hidden', kbFloating || kbModal.classList.contains('hidden'))
  if (btnKbDock) {
    btnKbDock.textContent = kbFloating ? 'Acoplar' : 'Desacoplar'
    btnKbDock.title = kbFloating ? 'Volver a acoplar al lateral' : 'Desacoplar y poder mover/redimensionar'
    btnKbDock.setAttribute('aria-pressed', kbFloating ? 'true' : 'false')
  }
}

function kbToggleFloating() {
  if (!kbModal) return
  if (!kbFloating) {
    const rect = kbModal.getBoundingClientRect()
    kbWindowState = kbClampWindowState({
      left: Math.max(12, rect.left - 18),
      top: Math.max(42, rect.top + 24),
      width: rect.width,
      height: Math.min(rect.height, window.innerHeight - 54)
    })
  }
  kbFloating = !kbFloating
  kbApplyWindowState()
  kbSaveWindowState()
}

function kbStartPointerAction(event, mode) {
  if (!kbFloating || event.button !== 0) return
  ...
  const move = (moveEvent) => {
    if (mode === 'drag') {
      kbWindowState.left = start.left + moveEvent.clientX - start.x
      kbWindowState.top = start.top + moveEvent.clientY - start.y
    } else {
      kbWindowState.width = start.width + moveEvent.clientX - start.x
      kbWindowState.height = start.height + moveEvent.clientY - start.y
    }
    kbApplyWindowState()
  }
  const stop = () => {
...
function kbSection(title, target = kbBody) {
  const div = document.createElement('div')
...
function kbSetTool(tool) {
  const isChat = tool === 'chat'
  const isFicha = tool === 'ficha'
  kbChatView?.classList.toggle('hidden', !isChat)
  kbFichaView?.classList.toggle('hidden', !isFicha)
  kbShortcutView?.classList.toggle('hidden', isChat || isFicha)
  if (kbToolHeading) kbToolHeading.textContent = isChat
    ? 'Agente conocimiento'
    : (isFicha ? 'Alta de fichas' : 'Alta de atajos')
  if (isChat) setTimeout(() => kbChatInput?.focus(), 40)
}

function kbChatSetStatus(text, kind = '') {
  if (!kbChatStatus) return
  kbChatStatus.textContent = text || ''
  kbChatStatus.className = `kb-chat-status${text ? '' : ' hidden'}${kind ? ` ${kind}` : ''}`
}

function kbChatCitationButton(citation) {
  const wrap = document.createElement('div')
  wrap.className = 'kb-citation'
  const marker = document.createElement('span')
  marker.className = 'kb-citation-marker'
  marker.textContent = citation.id || 'KB'
  const body = document.createElement('span')
  body.className = 'kb-citation-body'
  body.textContent = `${citation.title || citation.relPath || 'Ficha'}${citation.location ? ` · ${citation.location}` : ''}`
  wrap.appendChild(marker)
  wrap.appendChild(body)
  if (citation.relPath) {
    const open = document.createElement('button')
    open.type = 'button'
    open.className = 'kb-citation-open'
    open.textContent = 'Abrir ficha'
    open.addEventListener('click', () => window.api.kb.reveal(kbCwd, citation.relPath))
    wrap.appendChild(open)
  }
  if (citation.snippet) {
    const quote = document.createElement('div')
    quote.className = 'kb-citation-snippet'
    quote.textContent = citation.snippet
    wrap.appendChild(quote)
  }
  return wrap
}

function kbRenderChatHistory() {
  if (!kbChatMessages) return
  kbChatMessages.innerHTML = ''
  if (!kbChatHistory.length) {
    const welcome = document.createElement('div')
    welcome.className = 'kb-chat-welcome'
    welcome.innerHTML = '<div class="kb-welcome-mark">KB</div><h2>Pregunta al conocimiento del proyecto</h2><p>Este espacio está aislado de los demás proyectos. Selecciona las fuentes de la izquierda y pregunta por procedimientos, parámetros, decisiones o documentación.</p><div class="kb-suggestion-row"><button type="button" data-kb-suggest="¿Qué es lo más importante que debo saber de este proyecto?">Resumen ejecutivo</button><button type="button" data-kb-suggest="¿Qué errores o riesgos aparecen en la documentación?">Riesgos y errores</button></div>'
    welcome.querySelectorAll('[data-kb-suggest]').forEach((button) => {
      button.addEventListener('click', () => {
        if (!kbChatInput) return
        kbChatInput.value = button.dataset.kbSuggest || ''
        kbChatInput.focus()
      })
    })
    kbChatMessages.appendChild(welcome)
    return
  }
  for (const entry of kbChatHistory) {
    const row = document.createElement('div')
    row.className = `kb-chat-message ${entry.role === 'user' ? 'is-user' : 'is-assistant'}`
    const label = document.createElement('div')
    label.className = 'kb-message-label'
    label.textContent = entry.role === 'user' ? 'TÚ' : 'CONOCIMIENTO DEL PROYECTO'
    ...
    if (citations.length) {
      ...
      sourceList.className = 'kb-citations'
      const heading = document.createElement('div')
      heading.className = 'kb-citations-heading'
      heading.textContent = 'Fuentes consultadas'
      sourceList.appendChild(heading)
      citations.forEach((citation) => {
        sourceList.appendChild(kbChatCitationButton(typeof citation === 'string' ? { id: citation } : citation))
      })
      row.appendChild(sourceList)
    }
    kbChatMessages.appendChild(row)
  }
  kbChatMessages.scrollTop = kbChatMessages.scrollHeight
}

function kbRenderChatSources(fichas) {
  if (!kbChatSources) return
  const available = (fichas || []).filter((f) => f.active && !f.missing)
  if (kbChatSourceSelectionProject !== kbCwd) {
    kbChatSourceSelectionProject = kbCwd
    kbChatSourceSelection = new Set(available.map((f) => f.relPath))
  } else {
    const valid = new Set(available.map((f) => f.relPath))
    kbChatSourceSelection = new Set([...kbChatSourceSelection].filter((relPath) => valid.has(relPath)))
  }
  kbChatSources.innerHTML = ''
  if (!available.length) {
    const empty = document.createElement('div')
    empty.className = 'kb-source-empty'
    empty.textContent = 'Este proyecto aún no tiene fichas activas.'
    kbChatSources.appendChild(empty)
    return
  }
  for (const ficha of available) {
    const label = document.createElement('label')
    label.className = 'kb-source-choice'
    const check = document.createElement('input')
    check.type = 'checkbox'
    check.checked = kbChatSourceSelection.has(ficha.relPath)
    check.addEventListener('change', () => {
      if (check.checked) kbChatSourceSelection.add(ficha.relPath)
      else kbChatSourceSelection.delete(ficha.relPath)
    })
    const text = document.createElement('span')
    text.className = 'kb-source-choice-text'
    text.textContent = ficha.name
    text.title = ficha.relPath
    label.appendChild(check)
    label.appendChild(text)
    kbChatSources.appendChild(label)
  }
}

async function kbLoadChatHistory() {
  try {
    const data = await window.api.kb.history(kbCwd)
    kbChatHistory = data?.ok ? (data.history || []) : []
  } catch { kbChatHistory = [] }
  kbRenderChatHistory()
}

async function kbSubmitChatQuestion(questionOverride = '') {
  if (kbChatBusy) return
  const question = String(questionOverride || kbChatInput?.value || '').trim()
  if (!question) {
    kbChatSetStatus('Escribe una pregunta sobre este proyecto.', 'error')
    kbChatInput?.focus()
    return
  }
  if (!kbChatSourceSelection.size) {
    kbChatSetStatus('Selecciona al menos una ficha de la izquierda.', 'error')
    return
  }
  kbChatBusy = true
  if (kbChatInput) kbChatInput.value = ''
  kbChatHistory.push({ role: 'user', content: question })
  kbRenderChatHistory()
  kbChatSetStatus('Buscando en las fichas de este proyecto…')
  if (kbChatSend) { kbChatSend.disabled = true; kbChatSend.classList.add('is-loading'); kbChatSend.textContent = 'Consultando…' }
  try {
    const result = await window.api.kb.ask(kbCwd, question, [...kbChatSourceSelection], kbChatProject?.textContent || '')
    if (!result?.ok) throw new Error(result?.error || 'No se pudo consultar el conocimiento')
    kbChatHistory = result.history || [...kbChatHistory, {
      role: 'assistant', content: result.answer, citations: result.citations || []
    }]
    kbRenderChatHistory()
    kbChatSetStatus(result.grounded ? `${result.evidenceCount || 0} fragmentos consultados · confianza ${result.confidence}` : 'No se pudo verificar una respuesta con citas.', result.grounded ? 'ok' : 'warning')
  } catch (err) {
    kbChatHistory.push({ role: 'assistant', content: `No se pudo consultar este proyecto: ${err?.message || err}` })
    kbRenderChatHistory()
    kbChatSetStatus(err?.message || String(err), 'error')
  } finally {
    kbChatBusy = false
    if (kbChatSend) { kbChatSend.disabled = false; kbChatSend.classList.remove('is-loading'); kbChatSend.innerHTML = 'Preguntar <span>↵</span>' }
    kbChatInput?.focus()
  }
}

function kbPanelError(target, message) {
  if (!target) return
  target.innerHTML = ''
  const div = document.createElement('div')
...
function renderKbFichaPanel(data) {
  if (!kbBody) return
  kbBody.innerHTML = ''
  if (!data.ok) return kbPanelError(kbBody, data.error)

  kbSection('Fichas activas del proyecto')
  if (!data.fichas.length) {
    const div = document.createElement('div')
    div.className = 'kb-empty'
    div.textContent = data.claudeMdExists
      ? `El CLAUDE.md de ${kbCwd} no tiene fichas importadas todavía.`
      : 'Este proyecto aún no tiene fichas. Añade una fuente para empezar.'
  ...
  for (const ficha of data.fichas) {
    ...
    del.title = isPanelFicha ? 'Quitar la ficha y mandarla a la Papelera' : 'Quitar del conocimiento (el archivo no se toca)'
  ...
  if (data.fichas.length) {
    ...
    total.textContent = `Se cargan al abrir sesión: ${kbFmtSize(data.totalActiveBytes)} (~${(data.approxTokens / 1000).toFixed(1)}k tokens)`
  ...
  if (kbPendingApply.length) {
    ...
    btn.addEventListener('click', async () => {
      ...
      if (res.ok) {
        ...
        kbPendingApply = []
      } else kbProgressLine(`✗ ${res.error}`, 'kb-prog-err')
      await refreshKbPanel('ficha')
    })
    bar.appendChild(btn)
  ...
  if (data.fuentes.length) {
    kbSection('Fuentes originales', kbBody)
  ...
  drop.tabIndex = 0
  drop.textContent = kbBusy ? '⏳ Destilando…' : '⬇ Arrastra aquí un PDF, documento o audio · o haz clic para elegir'
  drop.addEventListener('click', () => { if (!kbBusy) kbPickAndDistillFiles() })
  drop.addEventListener('keydown', (e) => { if ((e.key === 'Enter' || e.key === ' ') && !kbBusy) kbPickAndDistillFiles() })
  ...
  urlInput.type = 'text'
  urlInput.placeholder = 'YouTube, web o documentación…'
  const urlBtn = document.createElement('button')
  urlBtn.textContent = kbBusy ? 'Destilando…' : 'Añadir enlace'
...
function renderKbShortcutPanel(data) {
  if (!kbShortcutBody) return
  kbShortcutBody.innerHTML = ''
  if (!data.ok) return kbPanelError(kbShortcutBody, data.error)
  kbSection('Atajos guardados', kbShortcutBody)
  ...
  for (const entry of shortcutEntries) {
    ...
    name.title = 'En la sesión, escribe el número o el título para recuperar este atajo'
    row.appendChild(name)
    kbShortcutBody.appendChild(row)
  }
  if (!shortcutEntries.length && !kbShortcutFormOpen) {
    const div = document.createElement('div')
    div.className = 'kb-empty'
    div.textContent = 'Todavía no hay atajos en este proyecto.'
    kbShortcutBody.appendChild(div)
  }
  if (!kbShortcutFormOpen) {
    const addBtn = document.createElement('button')
    addBtn.className = 'kb-primary-action'
    addBtn.textContent = '＋ Crear atajo'
    addBtn.addEventListener('click', () => { kbShortcutFormOpen = true; refreshKbPanel('shortcut') })
    kbShortcutBody.appendChild(addBtn)
    return
  }
  ...
  titleInput.placeholder = 'Título o pregunta frecuente'
  const bodyInput = document.createElement('textarea')
  bodyInput.rows = 7
  bodyInput.placeholder = 'Escribe la respuesta preparada: pasos, solución, procedimiento o criterio.'
  const relLabel = document.createElement('div')
  relLabel.className = 'kb-empty'
  relLabel.textContent = 'Fichas relacionadas (opcional):'
  const relBoxes = []
  ...
  saveBtn.className = 'kb-primary-action'
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    await refreshKbPanel('shortcut')
  })
  const cancelBtn = document.createElement('button')
  cancelBtn.className = 'kb-secondary-action'
  cancelBtn.textContent = 'Cancelar'
  cancelBtn.addEventListener('click', () => { kbShortcutFormOpen = false; refreshKbPanel('shortcut') })
  ...
  kbShortcutBody.appendChild(form)
}

async function refreshKbPanel(tool = kbActiveTool) {
  let data
  try {
  ...
  if (kbChatProject) kbChatProject.textContent = kbCwd.split('/').filter(Boolean).pop() || kbCwd
  kbRenderChatSources(data.fichas)
  if (tool === 'chat') return
  if (tool === 'shortcut') renderKbShortcutPanel(data)
  else renderKbFichaPanel(data)
}
...
function closeKbPanel() {
  ...
  kbDivider?.classList.add('hidden')
}

async function openKbPanel(tool = 'chat') {
  kbCwd = await kbResolveCwd()
  if (!kbCwd) {
    showStatus('Elige primero un proyecto para usar su conocimiento.', 'warn', 3500)
    return false
  }
  kbActiveTool = tool
  ...
  kbSetTool(tool)
  kbChatSetStatus('')
  kbChatSourceSelectionProject = ''
  kbChatSourceSelection = new Set()
  ...
  kbApplyWindowState()
  await kbLoadChatHistory()
  await refreshKbPanel(tool)
  if (tool === 'chat') kbChatInput?.focus()
  return true
}

async function kbPickAndDistillFiles() {
  const paths = await window.api.pickFile()
  if (!paths?.length) return
  for (const filePath of paths) {
    ...
    const added = await window.api.kb.addFile(kbCwd, filePath)
    if (!added.ok) {
      kbProgressLine(`✗ ${name}: ${added.error}`, 'kb-prog-err')
      continue
    }
    kbProgressLine(`+ fuente guardada: ${added.relPath}`)
    await kbRunDistill({ kind: 'file', relPath: added.relPath })
...
if (btnKb) btnKb.addEventListener('click', () => openKbPanel('chat'))
if (btnKbFicha) {
  btnKbFicha.addEventListener('click', async () => {
    if (!await openKbPanel('ficha')) return
    kbProgressLine('Selecciona un PDF, documento o audio para convertirlo en ficha…')
    await kbPickAndDistillFiles()
  })
}
if (btnKbShortcut) {
  btnKbShortcut.addEventListener('click', async () => {
    if (!await openKbPanel('shortcut')) return
    kbShortcutFormOpen = true
    await refreshKbPanel('shortcut')
    kbShortcutBody?.querySelector('.kb-shortcut-form input')?.focus()
  })
}
kbWindowState = kbReadWindowState()
kbFloating = !!kbWindowState.floating
kbApplyWindowState()
btnKbDock?.addEventListener('click', kbToggleFloating)
kbToolHeader?.addEventListener('pointerdown', (event) => kbStartPointerAction(event, 'drag'))
kbResizeHandle?.addEventListener('pointerdown', (event) => kbStartPointerAction(event, 'resize'))
kbDivider?.addEventListener('pointerdown', (event) => {
  if (kbFloating || event.button !== 0) return
  event.preventDefault()
  const startX = event.clientX
  const startWidth = kbModal.getBoundingClientRect().width
  const rowWidth = document.getElementById('terminal-row')?.getBoundingClientRect().width || window.innerWidth
  const move = (moveEvent) => {
    const width = Math.min(Math.max(startWidth + (startX - moveEvent.clientX), 420), Math.round(rowWidth * 0.68))
    kbWindowState = kbWindowState || kbReadWindowState()
    kbWindowState.dockWidth = width
    kbModal.style.flexBasis = `${width}px`
  }
  const stop = () => {
    document.removeEventListener('pointermove', move)
    kbSaveWindowState()
...
kbChatAllSources?.addEventListener('click', () => {
  kbChatSources?.querySelectorAll('input[type="checkbox"]').forEach((input) => {
    input.checked = true
    input.dispatchEvent(new Event('change'))
  })
})
kbChatClear?.addEventListener('click', async () => {
  if (!window.confirm('¿Borrar la conversación de conocimiento de este proyecto?')) return
  const result = await window.api.kb.clearHistory(kbCwd)
  if (result?.ok) {
    kbChatHistory = []
    kbRenderChatHistory()
    kbChatSetStatus('Conversación borrada.', 'ok')
  }
})
kbChatForm?.addEventListener('submit', (event) => {
  event.preventDefault()
  kbSubmitChatQuestion()
})
