...
- **Túnel y espejo LAN** (`tech/tech_lan_tunel_espejo_2026_08_15.md`): binario externo que la app spawnee → ruta absoluta vía `main/cli-resolver.js` (la empaquetada no hereda `/usr/local/bin`). El ESPEJO escribe RAW al PTY del host —excepción deliberada a `sanitizeChannelText`: es un terminal, sanear mata las teclas de control— y su facade DESENGANCHA en `kill()`, jamás mata el PTY del host; la puerta es la invitación temporal. Se abre con QR de 1 uso/90 s y la reconexión vive de un renewal no encadenable con techo de 4 h que NO es deslizante. El resto (audio, vistas, device-flow, diagnóstico del móvil) en la ficha.
...
- Las allowlists `SAFE_*` de `save-app-config` viven en `main/app-config-allowlists.js` y **descartan en silencio**: campo de config nuevo que envíe el renderer → añadirlo ahí, o desaparece al guardar sin error (`bugs/bug_lan_allowlist_urls_publicas_2026_08_13.md`). `authToken` y `enterprise.*` siguen fuera a propósito.
...
- Los `<script>` sueltos del renderer comparten ámbito global: un `const` duplicado —o un `const { x } = ...` contra la `function x` de otro fichero— mata la página entera. Lo caza `tests/renderer-global-scope-collisions.test.js`; el historial, en `bugs/bug_scripts_renderer_ambito_global.md`. **Y `npm run verify` da OK con la página muerta: mide que el proceso vive, no que el JS se ejecutara — arrancar ≠ funcionar.**
...
- El worktree de sesión nace de HEAD, así que el conocimiento sin commitear miente en las dos direcciones: una ficha nueva es invisible y **un borrado es inmortal** (`bugs/bug_kb_conocimiento_zombi_2026_08_13.md`). El invariante se garantiza donde NACE el worktree (`prepareSessionWorkspace` comitea CLAUDE.md+`kb/`, nunca `-A`), no parcheando cada ruta de escritura; si ese commit no es posible, la sesión arranca SIN aislar y avisa. Nunca dejar mudo un fallo de `commitKbChanges`.
- "Aplicar a sesión" usa rutas ABSOLUTAS y `writePromptThenEnter`.
- **El conocimiento es OPCIONAL por carpeta** (`main/kb-prefs.js` → `userData/kb-prefs.json`, **default OFF desde 2026-08-16**: se enciende carpeta a carpeta): con la casilla CONOCIMIENTO apagada (picker o popover AGENTE) no hay pestañas Casos/Fichas. Nada puede asumir que existen; qué se ve lo decide `kb-tabs-state.js`, fuera del renderer, porque la suite corre sin Electron. La barra PERSONALIDAD+CONOCIMIENTO del picker solo se ve en el paso de PROYECTO (vale para el proyecto entero; dentro se cambia en AGENTE) — detalle en `tech/runbook_kb_conocimiento.md` § 2026-08-16.
...
- **El bridge lo arranca launchd, no la app**, así que un reboot lo dejaba conectado solo. Por eso STOP es PERSISTENTE y START rehabilita: el estado de encendido vive en el override store de launchd, **no en la config**. Y `autoReply` decide si RESPONDE, que es cosa distinta de estar vivo. Plist, comandos medidos y por qué no se parsea la salida de launchd: `tech/runbook_whatsapp_bridge_ciclo_vida.md`.
