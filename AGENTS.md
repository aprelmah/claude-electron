...
- **Túnel y espejo LAN** (2026-08-15, `tech/tech_lan_tunel_espejo_2026_08_15.md`): binario externo que la app spawnee → ruta absoluta vía `main/cli-resolver.js` (la empaquetada no hereda `/usr/local/bin`; el ENOENT solo muerde empaquetado). El ESPEJO escribe RAW al PTY del host — excepción deliberada a `sanitizeChannelText` (es un terminal, sanear mata las teclas de control); la puerta es la invitación temporal y el facade en `session.ptyProcess` DESENGANCHA en `kill()`, jamás mata el PTY del host. Los transcripts de audio SÍ se sanean, con el ENTER en escritura aparte. La vista Chat de `lan-client.html` no pinta stream crudo: "ver el terminal" = vista terminal o `lan-mirror.html`. **El espejo se abre con QR (2026-08-16, `b597617`): invite de 1 uso/90 s; la reconexión vive de un renewal que SOLO emite el servidor por el WS, no encadenable, techo 4 h** — ese techo cuenta desde que se ESCANEA el QR y no es deslizante: el WS abierto aguanta lo que sea, pero pasadas las 4 h el primer corte es definitivo. Detalle en la misma ficha (§ 2026-08-16 y § 2026-08-21, que trae el diagnóstico en el móvil, el QR que se auto-renueva y el patrón de inyectar un módulo testeado en la página remota sin abrir endpoint).
...
- Los `<script>` sueltos del renderer comparten ámbito global: un `const` duplicado mata la página entera —también un `const { x } = ...` contra una `function x` de otro fichero, que reincidió el 2026-08-21 dejando el picker sin proyectos ni personalidades. **Ya NO depende de que alguien lo recuerde**: `tests/renderer-global-scope-collisions.test.js` lo caza (`bugs/bug_scripts_renderer_ambito_global.md`). La lógica de renderer que importe se extrae a módulo testeable. Y ojo: `npm run verify` da OK con la página muerta —mide que el proceso vive, no que el JS se ejecutara: **arrancar ≠ funcionar**.
